...
def get_weather_record(request):
   ...
   for row in results:
      ...
      temp = row[3]
      humidity = row[4]
      if extract_date_from_timestamp(timestamp) not in weather_details:
        weather_details[extract_date_from_timestamp(timestamp)] = {"location" : location, "description" : description, "temp" : temp, "humidity" : humidity}
      db.close()  # disconnect from server
